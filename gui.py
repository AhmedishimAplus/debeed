...
class DebeedApp:
    ...
    def _build_btn_bar(self):
        ...
        # Created but not packed — revealed once the saved filter URL exists
    ...
    def _refresh_page(self):
        ...
        next checkpoint/wait and performs the reload on its own thread (Playwright
        is not thread-safe, so the GUI never touches the page directly)."""
    ...
    def _on_url_saved(self):
        """Hook from run.main(): the saved filter URL now exists, so reveal the
